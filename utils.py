...
# MERGE_DATASET_PATH = './data/random split-merge-0.9'        # when run under the reducing training data setting
# MERGE_DATASET_PATH = './data/random split-merge-all-folds'  # when train on all the datasets, 5-fold
...
def split_train_test_data(all_feature_file_name = ALL_FEATURE_PATH,
    ...
    if feature_name in ['Avalon', 'Morgan', 'AtomPair']:
        ...
        # feature_df = all_feature_df[all_feature_df.columns[0:120]]
    else:
        feature_df = pd.read_csv('./data/' + feature_name +'.txt', dtype={'RTECS_ID': str})



    print('According to {0} and {1}, spliting toxicity label {2} and molecular feature {4} in file {3}...'.format(
        trn_split_file_name,tst_split_file_name,target_file_name,all_feature_file_name,feature_name
    ))
    # adding finger feature into the tail, 80081 * (1+1+59+1024)
    data_df = pd.merge(label_df, feature_df, on='RTECS_ID')
    print(f'--- dataset scale：{data_df.shape}，RTECS_ID 1-d + SMILES 1-d + Endpoints 59-d + feature 1024-d')
    ...
    print('Dataset processed over!')
    print('The merged training file and testing file of toxicity label and molecular feature {2} '
          'were saved into {0} and {1}.'.format(
...
def endpoint_decomposition(target_file_name = TARGET_PATH):
    ...
    print(f'--- There are {len(endpoints)} End Points ---')
    print(f'--- Species have {num_species} kinds:',attribute['species'])
    print(f'--- Routes have {num_routes} kinds:', attribute['routes'])
    print(f'--- Records have {num_records} kinds:', attribute['records'])
...
class ToxicityDataset(Dataset):
    def __init__(self,
        ...
        print(f'Checking whether {self.phase} exists...')
        if not os.path.exists(dataset_file_path):
            ...
            print(f'{self.phase}  is ready now！')
        ...
        target_nan = self.dataset[:, 2:61].astype('float32')  # missing mearsuments are nan

        # input feature，[num_samples, 1024], float
        ...
        # the mask of target，[num_samples, 59]，int,  the mising measurments are set to 0
